...
export function PosDataProvider({ children }: { children: React.ReactNode }) {
  ...
  const recordSale = async (
    ...
    try {
      // Resolve shift/employee context if available, but do not block sales when absent
      let resolvedEmployeeId = employeeId;
      let resolvedShiftId = shiftId;

      if (shiftId) {
        const explicitShift = shifts.find((s) => s.id === shiftId) || null;
        if (!explicitShift || explicitShift.status !== "active") {
          throw new Error(
            "Shift not found or not active. Please ensure you have an active shift before making a sale."
          );
        }
        resolvedShiftId = explicitShift.id;
        resolvedEmployeeId = resolvedEmployeeId || explicitShift.employeeId;
      } else if (employeeId) {
        const employeeShift = getActiveShiftForEmployee(employeeId);
        if (employeeShift) {
          resolvedShiftId = employeeShift.id;
        }
      } else {
        const anyShift = getActiveShiftForEmployee();
        if (anyShift) {
          resolvedShiftId = anyShift.id;
          resolvedEmployeeId = anyShift.employeeId;
        }
      }
