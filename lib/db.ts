...
const DATABASE_NAME = "boilerplate_dashboard_db";
...
const withProductMeasurementDefaults = (product: Product): Product => {
  ...
  const unitIncrement =
    typeof rawIncrement === "number" && rawIncrement > 0
      ? rawIncrement
      : saleType === "weight"
      ? DEFAULT_WEIGHT_INCREMENT
      : DEFAULT_ITEM_INCREMENT;
...
export const productsApi = {
  getAll: async (
    ...
    try {
      ...
      return (productsWithCategory as Product[]).map(
        withProductMeasurementDefaults
      );
    } catch (error) {
      console.error(
  ...
  getById: async (id: string): Promise<Product | undefined> => {
    try {
      ...
      return product ? withProductMeasurementDefaults(product) : undefined;
    } catch (error) {
  ...
  add: async (product: Product): Promise<string> => {
    try {
      ...
      const normalized = withProductMeasurementDefaults(product);
      await db.products.add(normalized);
      console.log("Product added successfully:", normalized.id); // Debug log
  ...
  update: async (product: Product): Promise<void> => {
    try {
      const db = getDB();
      const normalized = withProductMeasurementDefaults(product);
      console.log("Updating product:", normalized); // Debug log
      const updated = await db.products.update(normalized.id, normalized);
      if (updated) {
        console.log("Product updated successfully:", normalized.id); // Debug log
      } else {
        console.warn("No product found to update with ID:", normalized.id); // Debug log
      }
    } catch (error) {
  ...
  getByCategory: async (categoryId: string): Promise<Product[]> => {
    try {
      const db = getDB();
      const items = await db.products.where("categoryId").equals(categoryId).toArray();
      return items.map(withProductMeasurementDefaults);
    } catch (error) {
      console.error(
